...
const schema = z.object({
	oldPassword: z.string().min(8, 'Mot de passe requis'),
	newPassword: z
		.string()
...
export default function PasswordChangeFr () {
	...
	const onSubmit = async ({ oldPassword, newPassword }) => {
		...
		try {
			...
			setSuccess('Mot de passe changé. Veuillez vous reconnecter.')
			reset()
		} catch (err) {
			setLoading(false)
	...
	return (
		<div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-rose-100 to-blue-100 px-4">
			<div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md">
				<h1 className="text-2xl font-bold mb-6 text-center text-rose-700">Changer le mot de passe</h1>
				{error && (
